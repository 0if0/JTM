...
public class JtmImportJUnitRecorderTest {
    ...
    public void postBuild_expandsParameterizedRunName() throws Exception {
        ...
        FreeStyleProject p = j.createFreeStyleProject("jtm-import-params");
        p.addProperty(new ParametersDefinitionProperty(
            new StringParameterDefinition("version", "0.0.0")
        ));
        p.getBuildersList().add(writeSampleJUnitFile(xml));
        p.getPublishersList().add(new JtmImportJUnitRecorder(
            "target/surefire-reports/TEST-sample.xml",
        ...
        j.assertBuildStatusSuccess(
            p.scheduleBuild2(0, new ParametersAction(new StringParameterValue("version", "1.2.3"))));

        List<TestRun> recent = store.findRecentRuns(20);
        Optional<TestRun> imported = recent.stream()
            .filter(r -> "jtm-import-params".equals(r.getJobName()))
            .findFirst();
        assertThat(imported).isPresent();
        assertThat(imported.get().getName()).isEqualTo("Release 1.2.3");
    }
}
